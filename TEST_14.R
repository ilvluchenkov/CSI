...
DF <- inner_join(subs_csi_test, df_data_voice, by = c("SK_ID"="SK_ID", "CONTACT_DATE"= "T_DATE"))
DF[is.na(DF)] <- 0
...
setDT(CSI_test)
CSI_test <- CSI_test[, lapply(.SD, sum), by = c("SK_ID", "CSI", "CONTACT_DATE", "CELL_LAC_ID")]
...
read_subs_bs_consumption_test <- function(subs_bs_consumption_test){
  ...
  subs_bs_consumption_test <- subs_bs_consumption_test[, lapply(.SD, sum), by = c("SK_ID","CELL_LAC_ID")]
...
read_subs_features_test <- function(subs_features_test){
  ...
  subs_features_test$`COM_CAT#24` <- NULL
  subs_features_test$`COM_CAT#25` <- NULL
  subs_features_test$`COM_CAT#26` <- NULL
  subs_features_test$`COM_CAT#32` <- NULL
  subs_features_test$`COM_CAT#33` <- NULL
...
clc()
#subs_features_test <- subs_features_test[,c(1,7,9,12:14,17,28:32)]

#--------------------------FEATURES + CONSUME BY SUBS AT 3 MONTH BEFORE RESPONDENT--------------------------------------------------------------------------------------------------------------------------------------

DF$CELL_LAC_ID <- NULL
x_test_tbl <- inner_join(CSI_test,subs_features_test,  by = c("SK_ID"= "SK_ID"))



x_test_tbl$CONTACT_DATE <- NULL
x_test_tbl$CELL_LAC_ID <- NULL
x_test_tbl <- as.data.frame(lapply(x_test_tbl, function(x) as.numeric(as.character(x))))
setDT(x_test_tbl)
x_test_tbl <- x_test_tbl[, lapply(.SD, sum), by = c("SK_ID", "CSI")]
x_test_tbl <- as.data.frame(x_test_tbl)
x_test_tbl <- apply(x_test_tbl, 2, function(x) gsub("-Inf", 0, x))


clc()
#
x_test_tbl <- inner_join(x_test_tbl,subs_bs_consumption_test,  by = c("SK_ID"= "SK_ID"))
...
# setDT(x_test_tbl)
# x_test_tbl <- x_test_tbl[, lapply(.SD, mean), by = c("SK_ID", "CSI")]
# 
# #x_test_tbl <- as.data.frame(x_test_tbl)
# x_test_tbl[is.na(x_test_tbl)] <- 0
# 
# 
# x_test_tbl <- inner_join(subs_csi_test, x_test_tbl, by = c("SK_ID"= "SK_ID", "CONTACT_DATE" = "CONTACT_DATE"))
# 
# x_test_tbl$CSI.y <- NULL
# 
# 
# setDT(x_test_tbl)
# x_test_tbl <- x_test_tbl[, lapply(.SD, log), by = c("SK_ID", "CSI.x", "CONTACT_DATE")]
# x_test_tbl <- as.data.frame(x_test_tbl)
# x_test_tbl <- apply(x_test_tbl, 2, function(x) gsub("-Inf", 0, x))
